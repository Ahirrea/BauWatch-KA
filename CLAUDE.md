...
- **Feature-Ideen laufen über das Refinement, nicht direkt in Code.** Eine nicht-triviale Idee — auch als Frage formuliert („Können wir X?") — wird erst nach `docs/FEATURE-REFINEMENT.md` verfeinert: Bestandsaufnahme, Spannungen zu den Nicht-Zielen, Optionen mit Empfehlung, Weichen von der Ideengeberin entscheiden lassen, **eigene Datei** `docs/features/F-<Nr>-<kurz-titel>.md` (aus `docs/features/_vorlage.md`) + Zeile in `docs/features/README.md` (dort und **nur** dort lebt der Status). **Umsetzung erst auf ausdrückliches grünes Licht** — ein vorgegebener Feature-Branch-Name ist keins. Kleine Fixes und technische Aufgaben laufen weiter direkt über `BACKLOG.md`.
...
`README.md` (Setup, Action, „Änderungen nachvollziehen"), `docs/SPEC.md` (Produktziel & Nicht-Ziele), `docs/ADR-001-statisches-hosting.md` (Architekturentscheidung), `docs/BACKLOG.md` (Status je technischer Aufgabe), `docs/features/` (ausgearbeitete Feature-Ideen, je Feature eine Datei; `README.md` darin = Übersicht + Status), `docs/FEATURE-REFINEMENT.md` (fester Prozess: Idee → umsetzungsreife Spec).
